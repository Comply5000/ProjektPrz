    ...
    <form @submit.prevent="send">
      ...
      <div class="data-filter">
        ...
        <input
          ...
          :class="{ 'is-invalid': formErrors.dateFrom }"
        />
        <div v-if="formErrors.dateFrom" class="invalid-feedback">
          {{ formErrors.dateFrom }}
        </div>
        ...
        <input
          ...
          id="endDate"
          v-model="form.dateTo"
          :class="{ 'is-invalid': formErrors.dateTo }"
        />
        <div v-if="formErrors.dateTo" class="invalid-feedback">
          {{ formErrors.dateTo }}
        </div>
      </div>
      ...
      <div class="image-input">
        ...
        <button class="reset-image-button" type="button" @click="resetImage">
          Resetuj obraz
        </button>
      </div>
...
export default {
  ...
  methods: {
    async send() {
      ...
      const formattedDateTo = momentDateTo.isValid()
        ? momentDateTo.toISOString()
        : "";
      const momentDateFrom = moment(this.form.dateFrom, "YYYY-MM-DD");
      const formattedDateFrom = momentDateFrom.isValid()
        ? momentDateFrom.toISOString()
        : "";
      ...
      if (this.isEditMode) {
        ...
        axios
          .put(`/offers/${offerId.slice(1)}`, formData, {
            headers: {
            ...
            if (error.response && error.response.data) {
              ...
              if (serverErrors) {
                this.formErrors.name = serverErrors.Name
                  ? serverErrors.Name[0]
                  : "";
                this.formErrors.description = serverErrors.Description
                  ? serverErrors.Description[0]
                  : "";
                this.formErrors.dateTo = serverErrors.DateTo
                  ? serverErrors.DateTo[1]
                  : "";
                this.formErrors.dateFrom = serverErrors.DateFrom
                  ? serverErrors.DateFrom[1]
                  : "";
                this.formErrors.type = serverErrors.DateFrom
                  ? serverErrors.DateFrom[1]
                  : "";
                console.log(this.formErrors);
              } else if (
                serverTitle &&
                serverTitle.includes("oferta o takiej nazwie już istnieje")
              ) {
    ...
    fetch(offerId) {
      ...
      axios
        ...
        .then((response) => {
          ...
          const momentWarsawTimeDateTo =
            momentDateTotimeoffset.tz("Europe/Warsaw");
          this.form.dateTo = momentWarsawTimeDateTo.format("YYYY-MM-DD");

          const momentDateFromtimeoffset = moment(response.data.dateFrom);
          const momentWarsawTimeDateFrom =
            momentDateFromtimeoffset.tz("Europe/Warsaw");
...
body {
  background-color: rgb(
    ...
    160,
    96
  ); /* Jasnozielony kolor tła tylko dla sekcji wokół formularza */
}
